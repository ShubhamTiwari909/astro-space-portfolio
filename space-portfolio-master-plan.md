...
### Phase 4 — The Spine (implementation corrections)

| Date | Phase | Decision / Deviation | Reason |
|------|-------|---------------------|--------|
| 2026-09-04 | 4 | **§19.2a's import-discipline advice is wrong when R3F is present.** Named three.js imports do not shrink the bundle | R3F imports three wholesale to build its JSX element catalogue. A hand-rolled Catmull-Rom sampler written specifically to drop three's `Curve` hierarchy measured *larger*, and was reverted. Measured floor: ~229KB gz for three + R3F, ~2KB for our scene code |
| 2026-09-04 | 4 | **Bloom cut; scene-chunk budget corrected 230KB → 250KB** | The 230KB figure assumed tree-shaking *and* included a bloom pass; neither holds. Bloom is third on §26.2's own cut list, so it went rather than the budget being raised to hide it. 250KB is measured reality plus room for 4b's set pieces. Recorded in `PERF.md` |
| 2026-09-04 | 4 | **No drei dependency at all.** Nebula and stars are hand-written shaders rather than `Points`/`PointMaterial`/`useTexture` | The §19.2a allowlist existed to cap drei's cost; writing the two shaders directly removes the dependency entirely and needs no texture asset, so the nebula can be band-tinted from a uniform with zero image bytes |
| 2026-09-04 | 4 | **`ScrollDriver` stayed an Astro script** (decided in Phase 2) and the scene reads `scrollState` directly | Confirmed correct under load: the scene's 230KB chunk sits entirely off the critical path, which did not move between Phase 3 and Phase 4 (2.4KB both) |
| 2026-09-04 | 4 | Scene exposes `data-scene-tier` / `-dpr` / `-stars` | Test surface, not debug cruft: §34.3's tier-matrix test has to assert the scene mounted at the right fidelity, and this is the only way to observe that from outside the React tree |
| 2026-09-04 | 4 | **BUG FIXED: throttled frames triggered permanent degradation** | A backgrounded tab produces frames hundreds of ms apart, indistinguishable from a device that cannot cope. Since degradation is one-way by design, a visitor who switched tabs for a few seconds returned to a permanently degraded scene. Deltas above 100ms are now discarded as stalls, and they reset the rolling mean rather than poisoning it |
| 2026-09-04 | 4 | **BUG FIXED: `window.innerWidth` can be 0, pinning the tier to `low`** | It reports 0 in a background or prerendering tab and in a `display:none` iframe. Detection is cached for the page's life, so this was unrecoverable. Width is now the max of `innerWidth`, `documentElement.clientWidth` and `screen.width`, and **0 is treated as unknown rather than tiny** — an unknown width must never downgrade, because the frame governor is the safety net for an optimistic guess |
| 2026-09-04 | 4 | The delete-the-canvas gate asserts **absence** from the served HTML, not `aria-hidden` | Stronger than the plan assumed: `DeepFieldScene` returns null during SSR, so the scene and canvas are created entirely client-side. A visitor with no JS, no WebGL, or `saveData` never receives the element at all |
| 2026-09-04 | 4 | Added `tests/flight-path.mjs` — spline assertions run in node | The camera path is the centrepiece and the hardest thing to verify visually, since browsers only paint the top of a document. The maths is fully testable: keyframe order, exact keyframe interpolation, continuity, forward travel, and the final beat looking back |

### Phase 4b — The Set Pieces (implementation corrections)

| Date | Phase | Decision / Deviation | Reason |
|------|-------|---------------------|--------|
| 2026-09-04 | 4b | **ConstellationGlobe cut entirely** | It would encode exactly what the skills SVG map already encodes — skills, tiers, relationships, labels — and the SVG does it for ~2KB, accessibly, with labels always visible. Spending ~6KB plus per-frame DOM label projection to convey the same information *less* accessibly is what P2 explicitly forbids. The globe was the one set piece with a DOM equivalent already shipped |
| 2026-09-04 | 4b | **Planet surfaces are procedural GLSL, not AI-generated equirectangular textures** | Three consequences, all favourable: zero image bytes where the plan budgeted ~120KB per planet plus a still per planet for the fallback; the hue comes from the same `plateHueForIndex` the CSS fallback sphere uses, so the 3D planet and its DOM counterpart are the same colour *by construction* rather than by two artists agreeing; and there is nothing to decode on mobile, which is where this beat was most at risk. Cuts six entries from §29's raster inventory |
| 2026-09-04 | 4b | **The planned `WorldViewer` React island was replaced by `lib/worldFocus.ts`, a module singleton** | The island existed to tell the scene which project card the visitor is on. That needs no component, no hydration and no React state: two delegated listeners on `document` (`pointerenter`/`pointerleave` in capture, `focusin`/`focusout` bubbling) publish to a mutable object the scene reads inside `useFrame`. Keyboard parity comes free, because focus and hover go through the same path — and per-frame reads never touch React state |
| 2026-09-04 | 4b | **BUG FIXED: the nebula fogged the payoff beat.** Layers confined to z +112 → +12 | Spanning the whole flight put additive billboards between the camera and the planets during the Worlds beat — the payoff of the entire flight, and the longest dwell. A first correction to −14 was worse, not better: it placed a 200-unit billboard two units in front of the camera at z −16 and whited the beat out completely. Dust now sits only where the journey passes *through* it |
| 2026-09-04 | 4b | **The art layer recedes to `--art-strength: 0.32` under `html[data-scene-ready]`** | The art plates were designed in Phase 2 as the *entire* backdrop. With the scene drawing over them, two full-strength images competed and the planets lost. After handover the art's job is ground and depth, not co-equal imagery — and it still renders at full strength as the complete no-WebGL experience |
| 2026-09-04 | 4b | **The featured world's ring needs its own material; it cannot reuse the atmosphere shell's** | A fresnel term needs curvature. Every normal on a flat ring points the same way, so the term collapsed to a constant and the ring rendered as an opaque dark disc that cut across the planet and occluded the bodies behind it. The ring material shades across the ring's *width* instead, with a Cassini-style gap. Two follow-on corrections came from looking at it: additive blending over a 0.78-lightness base washes all hue out to grey (now 0.9 saturation at 0.58 lightness), and at 21° open the ring crossed the planet as a hard bright line rather than reading as a ring (now 30°) |
| 2026-09-04 | 4b | Planet system **pulled back and scaled down** (z −58/−74/−82/−94, radii 5.4/3.1/2.8/2.4); station moved to z −112, well off-axis | Both were found by rendering, not by reasoning. At z −50 the featured world subtended most of the frame and the others crowded its ring; at z −86 the station's wireframe tangled with the planet system in screen space and read as clutter rather than as a separate object |
| 2026-09-04 | 4b | Added `playwright-core` (dev-only) and `scripts/shoot-beats.mjs` | The harness browser pane does not composite while hidden, which starves `requestAnimationFrame`. A scroll-driven WebGL scene is unverifiable under that: the IntersectionObserver never fires so every section stays at `opacity: 0`, and the canvas holds a stale frame — the Worlds beat screenshotted pure black while `--scroll`, `data-band`, `--art-strength` and `data-scene-ready` were all provably correct. Driving real Chrome makes the flight reviewable, and §34.3's tier-matrix and runtime delete-the-canvas tests need this harness in Phase 6 regardless |

### Phase 5 — Engineering Showcase (implementation corrections)

| Date | Phase | Decision / Deviation | Reason |
|------|-------|---------------------|--------|
| 2026-09-04 | 5 | **Two internal contradictions in the plan resolved in R2's favour.** §14.3 says "~2,000 bodies" in its prose but "Hard-capped: 200 bodies" in its boundaries, and Phase 5's *Things NOT to implement yet* still forbids "No WebGL/R3F" while task 5 mandates a WebGL simulation | Both are Revision 1 text that R2 reversed and did not fully sweep. R2's decision log is explicit — Canvas 2D removed entirely, Orbit moved to WebGL, 200 → 2,000 bodies — so the cap is 2,000 and WebGL is correct. Recorded rather than silently picked |
| 2026-09-04 | 5 | **The simulation is a *restricted* n-body problem, and says so in the UI**: four masses interacting pairwise, 2,000 massless test particles integrated against them | True pairwise gravity across 2,000 bodies is 2M interactions per frame, needing Barnes-Hut or a GPGPU pass — more machinery than one exhibit justifies, and the plan's own instruction is to simplify the physics rather than add a dependency. The restricted problem is ~10,000 evaluations, measured at **0.10ms/step**, and still produces the thing worth looking at. Calling it "n-body" without qualification would have been the dishonest option, so the panel states the model in words |
| 2026-09-04 | 5 | **No component-library demos, and no `@zentauri-ui/zentauri-components` dependency.** I-01 is evidenced by two annotated excerpts of its *published type declarations*, plus its live docs and write-up | The real package is 22MB unpacked with eight peer dependencies including `recharts`, `framer-motion` and `react-icons` — an order of magnitude over the whole route budget. Reimplementing its components locally would be fabricated evidence, which §14.2 forbids outright. The types are better evidence anyway: `CopyButtonProps = Omit<CopyButtonBaseProps, 'renderIcon'>` and `framer-motion` confined to the `/animated` subpath *prove* the API-design and optional-motion claims, cost zero runtime bytes, and anyone can check them with `npm pack`. **Framer Motion was therefore never installed** |
| 2026-09-04 | 5 | I-02's excerpt comes from **this repository**, with its provenance printed | The work suite is not public, so there is nothing to excerpt honestly. Rather than invent a spec file, the panel shows the same discipline applied here — the delete-the-canvas gate — where the reader can open the whole file. Excerpts are a content collection, so dropping in a real spec is a JSON edit. `source` is a required schema field and always rendered: an excerpt whose provenance cannot be stated is not evidence |
| 2026-09-04 | 5 | `client:media="(min-width: 1024px)"` **alone**, not `client:visible` as well | Astro permits one client directive per component. Of the two §14.3 names, `client:media` is the one enforcing the hard boundary — verified: no Orbit JS is requested at 390px or 820px. What `client:visible` would have bought is reproduced inside the island and more precisely: no WebGL context until the container intersects, and the frame loop suspended when off-screen or the tab is hidden — which task 7 required regardless |
| 2026-09-04 | 5 | **Orbit stands the backdrop scene down while it runs** (`lib/sceneSuspend.ts`) | `/instruments` is the only route with two WebGL contexts, and sharing the GPU cost ~18ms per frame — enough for Orbit to trip its own 20ms degrade threshold because of work it does not do, and to report the *page's* frame time as the simulation's. The backdrop now suspends its frame loop (keeping its context and last frame, so nothing flickers). Frame rate went 51 → a locked 60 |
| 2026-09-04 | 5 | Telemetry reports **frame interval and simulation time separately** | The first version showed "Frame 16.7ms", which is the vsync interval, not work done — an instrument that overstates its own cost by 50× on a panel whose entire thesis is self-measurement. It now reads `Interval 17.1ms` beside `Sim 0.23ms`. The governor still watches the interval, because that is what rises when a device cannot cope |
| 2026-09-04 | 5 | Route JS held to **340KB (§26.2)**, not §14.5's 160KB | 160KB is a Revision 1 figure and is now impossible on *any* route: the persistent backdrop puts three + R3F (~229KB) on every page by design. What §14.5 was protecting — that the experiment costs almost nothing on top — is asserted directly instead: the Orbit island measures **4.8KB** against §14.3's 12KB |
| 2026-09-04 | 5 | **BUG FIXED (in the tests): the scene-chunk budget silently stopped measuring three.js** | Once Orbit also imported three, the bundler hoisted it into a shared chunk and `DeepFieldScene.*.js` fell from 233KB to 7KB. The single-file assertion went on passing at 3% of budget while the 229KB it existed to guard simply moved next door. Budgets now measure the **transitive import closure**, which cannot be dodged that way — and the true Phase 4b figure was 240.3KB, not the 233.0KB recorded |
| 2026-09-04 | 5 | **BUG FIXED: mobile downloaded the Orbit poster twice** | Astro server-renders an island's markup regardless of its client directive, so the island's `<img>` shipped at every width and Chrome fetched it even inside a `display:none` box — measured, two poster requests at 390px. The image now renders only after hydration; the host box already reserves its aspect ratio, so there is no layout shift |
| 2026-09-04 | 5 | **BUG FIXED: `initAnalytics()` was never called.** Wired in `BaseLayout` | Six components had carried `data-analytics` attributes since Phase 3, and the delegated listener that reads them did not exist at runtime — every declarative call site was inert. `track()` is still a no-op until Phase 8 supplies a provider, so nothing was lost, but wiring new call sites into a listener that never ran would have been theatre |
| 2026-09-04 | 5 | Initial conditions and framing set from **measurement, not taste** | Particles were seeded from r=8, inside the ring of masses at r 12–18, so they scattered off the attractors and were ejected: radius p90 reached 105 units against a 56-unit view, and most of the simulation was off-screen. Seeding outside the ring at full circular velocity gives p50 49 / p90 63, framed by `HALF_HEIGHT = 62`. The speed→colour ramp was likewise divided by a guessed 46 when measured p99 is 19, so it never left its blue end |
...
### Phase 7 — Performance (local pass; deploy-dependent items deferred)

| Date | Phase | Decision / Deviation | Reason |
|------|-------|---------------------|--------|
| 2026-09-05 | 7 | **BUG FIXED: 2.59MB of PNG shipped — 68% of the entire build** | Astro's `<Picture>` defaults `fallbackFormat` to PNG, and it produced that fallback by *up-converting* the 130KB webp source into four PNG variants: 1336KB, 710KB, 362KB and 181KB, with the bare `src` pointing at the largest. Nothing in the support matrix would ever fetch them — anything reaching the `<img>` instead of the avif or webp `<source>` is old enough that a 1.3MB PNG is not a kindness. `fallbackFormat="webp"` took the build from **5.12MB to 2.64MB** |
| 2026-09-05 | 7 | **BUG FIXED: `/transmissions` and `/dossier` were mounting the WebGL scene**, which §26.2 says they deliberately do not | Never implemented — `BaseLayout` defaulted `backdrop` on and Cosmos always rendered the scene. Measured, both routes fetched the 226KB chunk against 16KB and 20KB JS budgets: **1858% and 1486% over**. Cosmos now takes a `scene` prop. The art layer stays, because it is the designed backdrop in its own right and its images were already inside those routes' budgets |
| 2026-09-05 | 7 | **`MobileNav` converted from a React island to an Astro script** | The audit's whole purpose (task 1): 54.8KB of react-dom plus 4.6KB of react and scheduler to run a 1.3KB component. Once the scene left the reading routes it was their *entire* payload — 63.4KB against a 16KB budget, to toggle a menu. Its own doc comment already described it as "additive only" over static markup, which is exactly the ScrollDriver argument from Phase 2. `/transmissions` and `/dossier` went **297KB → 3.0KB** of JS |
| 2026-09-05 | 7 | **BUG FIXED: the scroll loop forced a synchronous reflow every frame** | `frame()` read `documentElement.scrollHeight` and called `getBoundingClientRect()` on every section — up to ten layout reads — then wrote custom properties to `<body>`, which invalidates style for the whole subtree. Read-after-write, sixty times a second, for geometry that only changes when the viewport or the document does. Offsets are now measured once and re-measured on resize and on a `ResizeObserver` over `<body>`; the loop reads only `scrollY`. The file's own comment had said "re-resolve rather than caching rects" |
| 2026-09-05 | 7 | **BUG FIXED: §24.2's save-data bail-out ran *after* the 227KB download.** New `client:capable` directive | `useSceneTier` documents that without WebGL, or with save-data set, "the art layer is the entire experience and no chunk is fetched". Under `client:idle` that was false: the browser fetched and parsed the whole scene chunk and only then decided to render nothing — so the precise group the bail-out protects paid its full cost. A local integration registers `client:capable`, which asks the two hard questions at idle and only then calls `load()`. Verified: with save-data set, **zero scene chunks are fetched and no canvas is created** |
| 2026-09-05 | 7 | Five shared `src/lib` modules merged into one `runtime` chunk | They were emitted as five separate files totalling under 3KB and always fetched together. Five round trips for 3KB is worse than one. Mobile home requests went 30 → 28, inside §26.2's budget |
| 2026-09-05 | 7 | **§26.2's mobile JS budget (190KB) and `/instruments` (215KB) are unreachable, and are recorded as a pinned deviation rather than raised or hidden** | The floor is architectural: three + R3F 227.0KB, react-dom 54.8KB, react and scheduler 4.6KB, scene and islands ~11KB — **~297KB**. The 190KB figure assumed a mobile-specific scene bundle that does not exist (one build, one chunk) and accounted for no react-dom at all, though the scene *is* a React component. It was unreachable the moment R2 chose R3F. The plan's rule is "cut scope, never raise the number", and both cuts that would work reverse a recorded decision — dropping the 3D on mobile reverses R2's explicit choice (its own R18 risk), and replacing R3F with raw three is a rewrite of the scene layer. **Owner decision; not taken unilaterally.** `tests/perf-audit.mjs` reports these two rows as OVER with a ceiling pinned to the measurement, so the gap stays visible and cannot grow |
| 2026-09-05 | 7 | Lighthouse asserts **accessibility 100 and CLS ≤ 0.02**; performance is recorded, not asserted | This machine gives headless Chrome no GPU, so every WebGL frame rasterises in software and a performance score measures SwiftShader rather than the site — which is what Phase 7 means by "local numbers are not authoritative". Accessibility and CLS are DOM-level and stable, so they are hard gates. `LH_ASSERT_PERF=1` turns the §26.1 thresholds on against the deployed URL |
| 2026-09-05 | 7 | Test servers replaced with an in-process static server (`tests/lib/serve.mjs`) | `astro preview` daemonises: killing the spawned process leaves the real server running, so every suite leaked a listener and the next suite failed to bind with a bare `ERR_CONNECTION_REFUSED` several steps later. A plain file server also serves exactly the bytes a CDN would, with no dev middleware in the path |
| 2026-09-05 | 7 | Two Phase 7 audit checks were wrong before the code was | The animation audit read every `transition-property` in the bundle and flagged `filter`, `backdrop-filter` and `content-visibility` — Tailwind's vocabulary for utilities nothing uses. It now considers only classes present in the built HTML. It also flagged `scrollProgress.ts` for layout reads that live in `measure()`; it now inspects the frame callback's body, not the file. A test that cries wolf is worse than no test |
| 2026-09-05 | 7 | **Deferred to the deployed site:** task 8's authoritative Lighthouse run, task 11's cache-header verification, and the 4× CPU / Slow 4G and 5-minute soak passes | All four need either the real host or real hardware. Local baselines are recorded in `PERF.md` so the deployed numbers have something to be compared against. Cache policy is exercised locally by the test server (hashed assets `immutable`, HTML `must-revalidate`) but that proves the intent, not the host |

### Phase 7 — follow-up: the dossier route

| Date | Phase | Decision / Deviation | Reason |
|------|-------|---------------------|--------|
| 2026-09-05 | 7 | **BUG FIXED: `/dossier` was unreachable from anywhere on the site.** The TopBar control now links to the page | Every control labelled "Dossier" or "Download résumé" — TopBar, Hero, Uplink — pointed at the PDF. The only `href="/dossier"` in the entire build was inside the `/resume` redirect stub, so the HTML résumé could only be reached by typing the URL. That defeats the route's whole rationale (§8.3: "HTML is indexable, linkable, and readable on a phone in a way a PDF is not") and contradicts §6.1's requirement that the page and the PDF both be one tap from every route. The TopBar now goes to the page with an `arrow-right`; the PDF stays one tap away via the Hero and Uplink buttons and the Download PDF button at the top of the dossier itself. No analytics event on the TopBar link — navigating to a page is not a download, and §28.1's event set is deliberately closed |
| 2026-09-05 | 7 | **`/dossier` now sets `backdrop={false}`**, per §8.3's "no cosmos behind it" | It had been rendering the full art layer. The page is now what §8.3 describes — a single column on a solid panel that reads as a document and prints as one — and drops ~34KB of art plates from the route. `scene={false}` is kept explicitly alongside it so re-enabling the backdrop later cannot quietly bring the 226KB WebGL chunk back to a reading route (§26.2) |

### Pre-deployment (Vercel)

| Date | Phase | Decision / Deviation | Reason |
|------|-------|---------------------|--------|
| 2026-09-05 | 7/8 | **D2 resolved without picking a domain: `site` is resolved from the environment**, not hardcoded | `SITE_URL` wins; otherwise `VERCEL_PROJECT_PRODUCTION_URL`, which Vercel injects on every build and which is always the *production* domain even inside a preview deployment — so preview builds never emit canonicals pointing at themselves. Falls back to localhost for a local build. The placeholder `https://example.com` had been baked into ten built files including the sitemap; there is now no placeholder left to forget |
| 2026-09-05 | 7/8 | **BUG FIXED: `/favicon.svg` 404'd on every page load.** Added | `BaseLayout` had referenced it since Phase 1 and the file never existed. Drawn as a ringed world in Ion Cyan on void — four shapes, because at 16px that is all that survives |
| 2026-09-05 | 7/8 | `robots.txt` is a **generated route**, not a file in `public/` | The `Sitemap:` line needs an absolute URL, so a static file would hardcode the domain in a second place and defeat the point of resolving it once. `src/pages/robots.txt.ts` derives it from `site`. Disallows `/dev/`, which already carries `noindex` — two lines of defence for a development surface that is publicly reachable |
| 2026-09-05 | 7/8 | `vercel.json` added: cache headers, `trailingSlash: false`, and the four legacy redirects as real 308s | Completes Phase 7 task 11. Vercel serves every static file with `must-revalidate` by default, which is right for HTML and wrong for content-hashed assets, so `/_astro/*` is pinned to `immutable` for a year. `trailingSlash: false` mirrors Astro's `trailingSlash: 'never'`. The redirects were already emitted as HTML stubs by Astro; as Vercel rules they become real 308s and are evaluated before the filesystem |
| 2026-09-05 | 7/8 | **No Vercel adapter.** The build stays `output: 'static'` | Phase 7 says it directly: "Do not add SSR or an adapter to 'improve' anything — static is already optimal here." Vercel detects Astro and serves `dist` |
| 2026-09-05 | 7/8 | `packageManager` pinned to the pnpm that produced the lockfile | Otherwise corepack on Vercel may resolve a different pnpm than the one the lockfile was written with |
| 2026-09-05 | 7/8 | Confirmed: **the build needs no Python and no browser** | `scripts/_art.py` (backdrop plates, `pnpm art`) and `scripts/generate-orbit-poster.mjs` (`pnpm poster`) are authoring tools run by hand. Every asset they produce is committed, so Vercel runs only `pnpm install && astro build` |

### Pre-deployment fixes (round two)

| Date | Phase | Decision / Deviation | Reason |
|------|-------|---------------------|--------|
| 2026-09-05 | 7/8 | **BUG FIXED: the flight never reached its own opening keyframe.** Section 0's blend is rescaled so scroll 0 maps to curve t 0 | `CameraRig` builds its curve parameter from `(bandIndex + bandBlend) / SEGMENTS`, and `computeBand` measures blend from the *viewport midpoint* — so at the top of the document the first section already owned half the screen and blend was ~0.45 before anyone scrolled. Measured: the camera sat at **z = 100.9 when §4.1 authors the opening beat at z = 120**. The composition the plan describes as First Light was never on screen. Rescaling section 0's blend fixes it without moving any other beat: every later section still reaches blend 1 exactly when the next takes the middle of the screen. Verified at 1440 and 390 — both now rest at exactly 120 |
| 2026-09-05 | 7/8 | **BUG FIXED: the station drew a wireframe streak through the hero type.** Distance fade added | At 232 units from the opening camera the Instrument Bay station was still drawing as a hairline, and it landed on "Tiwari" at 820px and across the lede at 390px — exactly what §9.1 forbids, since the hero is typography-led. It now fades out with distance, which is what the object should do anyway: a station 200 units away is not visible as a wireframe, and one you fly past should recede rather than pop |
| 2026-09-05 | 7/8 | **BUG FIXED: the mobile nav's scrim covered the menu it was opening** | The scrim sat at `--z-overlay` (60) while the nav is `--z-nav` (45). Because the nav's own z-index creates a stacking context, the sheet's `calc(--z-overlay + 1)` only ever counted *inside* the nav — so the scrim painted over the whole bar, sheet included, and the links it was meant to reveal were covered and unclickable. It now sits just below the nav that owns it. Hit-tested rather than visibility-tested: `elementFromPoint` at each link's centre returns the link |
| 2026-09-05 | 7/8 | **BUG FIXED: every mobile nav link was a bare fragment**, so the bar did nothing on any route but home | `#atlas`, `#log` and the rest only resolve on the one-page survey; from `/dossier` or `/instruments` they pointed at sections that do not exist. Now absolute (`/#atlas`), which behaves identically on home — a same-path fragment still scrolls without a reload — and correctly everywhere else. The two entries with routes of their own now link to the route, and Dossier was added to the sheet |
| 2026-09-05 | 7/8 | Icon set completed: `favicon.ico` and `apple-touch-icon.png`, generated from the SVG by `pnpm icons` | An SVG favicon alone is not enough in practice. Chrome requests `/favicon.ico` implicitly whatever the link tags say and caches the 404 — including the negative result, which is why a favicon added late appears not to work long after the file exists. Safari has never supported SVG favicons at all |
| 2026-09-05 | 7/8 | **Opening "inside a station" concept prototyped and reverted** | Owner asked whether a station in the hero would look good, then whether the opening could begin inside one. Both were built and reviewed against screenshots: a foreground hull silhouette with a lit aperture, sized from the camera frustum so it hugged the viewport edge at any aspect. It worked — the fly-through read correctly — but the owner judged the result not good enough, and it went. Recorded because the two bugs above were found *by* that work and are worth keeping: the camera never reaching keyframe 0, and the station streaking through the headline |


### Post-deployment performance pass

| Date | Phase | Decision / Deviation | Reason |
|------|-------|---------------------|--------|
| 2026-09-07 | 8 | **BUG FIXED: `tests/lib/serve.mjs` sent no `content-encoding`**, so every local Lighthouse and every byte-sensitive measurement taken through it was inflated ~3.7x | Its doc comment claimed it served "exactly the bytes a CDN would"; Vercel serves brotli. The scene chunk measured 880623B locally against 237731B deployed. Because Lighthouse's simulated throttling derives timings from observed transfer sizes, `/` scored **56 locally and 99 deployed on the same build**. Fixing the negotiation moved the local median from 81 to 97 with no change to the site. The instrument was wrong before the site was — the same failure mode as the Phase 4b budget test that stopped measuring three.js while still passing |
| 2026-09-07 | 8 | **`client:capable` now waits for the `load` event before its idle callback** | `requestIdleCallback` measures the main thread, not the network. This page has almost no blocking script, so the callback fired while the stylesheet, both preloaded fonts and the opening art plate were still in flight, and the scene's 212KB joined that queue. Lighthouse's own dependency tree named the longest chain on `/`: `/` → `DeepFieldScene.js` → `react-three-fiber.esm.js`. FCP was bimodal — ~1450ms when the fetch lost the race, ~2290ms when it won. The art layer has already painted (§17.1) and the opening beat is "nearly still" (§4.1), so the scene has nothing to gain from arriving early and a measurable cost |
| 2026-09-07 | 8 | **`SectionRail` converted from a React island to an Astro component**, `SectionRail.tsx` deleted | It cost the home route react-dom — 53KB and a 73ms long task — to add two attributes and one `scale` to a list of anchors the server already rendered. Worse, the rail is `hidden lg:flex`, so every phone paid the full bill for a component it cannot display, which is exactly what §24.2 exists to prevent. Same reasoning that retired the `MobileNav` island in Phase 7. React earns its place here once, for the scene, where the reconciler does real work. Verified: active index tracks 0→4→7, exactly one `aria-current`, fill scales 0→1, anchors work with JS disabled, hidden at 390px |
| 2026-09-07 | 8 | **BUG FIXED: the scene hydrated with a mismatch (React #418)** on every load. It now renders nothing until mounted on the client | Astro server-renders an island's markup whatever its client directive, so `DeepFieldScene` ran at build time with no `window`: the tier ladder fell back to `low` and R3F's `<Canvas>` emitted a wrapper containing no canvas. The client rendered a different tier and created the canvas imperatively, so React threw away the server tree it had just been handed. The scene is unavoidably client-only — it needs WebGL, a real `devicePixelRatio` and `matchMedia` to decide anything. Server renders nothing, first client render renders nothing, which is what makes them match. §25.0 already wanted the canvas absent from the HTML; this is that, enforced by React's own rules. Best practices 96 → 100 |
| 2026-09-07 | 8 | `--ff-body` moved from **Google's provider to Fontsource**, one static 400 face | Google serves Inter as a single variable file whatever weights are requested, so selecting weights against it changed nothing — the provider was the lever, not the weight list. Static latin 400 is **23.7KB against the variable's 48.4KB**, on the highest-priority resource on the critical path. One weight, because `<Font preload />` preloads *every* face of a cssVariable: shipping a 600 as well simply put both on the critical path. Nothing needed it — the body renders at 400 throughout and both `font-medium` uses are on the mono face |
| 2026-09-07 | 8 | Instrument Serif **split into two families**, roman and italic, so only the roman is preloaded | Same `<Font preload />` behaviour: the italic was 15.7KB at High priority, competing with the stylesheet and the LCP image, for a face used exactly twice on the site — a pull-quote in the observer's log and one in the dossier, both far below the fold. The roman keeps its preload because it sets the `h1` |
| 2026-09-07 | 8 | The one default-weight `<strong>` on `/dossier` became **`font-mono font-medium`** | It wraps a metric, and every other metric on the site is mono medium (`MetricBlock`, the hero readout). It was the only reason to ship a bold Inter, and making it consistent removed the need |
| 2026-09-07 | 8 | The opening art plate is **preloaded once per breakpoint**, and the pre-hydration rule gained a mobile variant | A single unconditional preload of the 2560px plate meant a phone fetched 28.5KB at the highest image priority on the page *and* then fetched the 1280px variant the moment `data-band` appeared. 28.5KB off the mobile critical path |
| 2026-09-07 | 8 | **Seven of the eight LQIPs replaced with a gradient in their own band hue**; only the opening plate keeps an inlined data URI | 11.8KB of base64 was 7.9KB of the brotli'd document — 31% of the very first render-blocking resource — for seven images that cannot be seen until the visitor has scrolled to their band. Emitting them as real files was tried first and **rejected: it put the mobile home route at 34 requests against §26.2's 28**, and §26.2 is cut-scope-not-raise-the-number. A gradient costs one request fewer than nothing and is the palette's own language (§3.3). Correction to §17.1, which states every plate carries an inlined LQIP: only the opening one now does |
| 2026-09-07 | 8 | Recorded: **deduplicating the LQIPs saved 86 bytes, not the ~8KB expected** | Each was emitted three times (base rule, active rule, mobile rule). Brotli collapses identical strings, so only the unique bytes ever mattered. The dedup was kept for the 24KB of parser input it removes, not for the wire. Also learned: a `background-image` in a plate's base rule is fetched immediately however `opacity:0` it is, while a custom property is not resolved until something *uses* it — that, not the file-versus-data-URI choice, is what defers a fetch |
| 2026-09-07 | 8 | Known, unfixed: **the hero `h1` is not an LCP candidate** | It starts at `opacity: 0` behind the `motion-safe` first-light reveal, so LCP resolves against the header wordmark (124x44px). This currently flatters the score, and the reveal is deliberate (§9.x), so the art direction was left alone — but the reported LCP is not measuring the hero, and a browser that treats animated-in text differently would move the number without the site changing |
